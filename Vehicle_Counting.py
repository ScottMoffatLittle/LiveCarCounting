    ...
    def count(self, frame):
        ...
        for _id, blob in list(self.blobs.items()):
            ...
            if (self.counting_line == None and \
                ...
                log.append({'blob_id': _id, 'count': self.vehicle_count, 'frames': blob.frames,'datetime': datetime.now()})
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ID', help='select an ID to recognize outputs')
    ...
    if args.record:
        ...
        #output_video_path= './videos/output{ID}.avi'.format(ID=str(args.ID))
        #output_video = cv2.VideoWriter(output_video_path, \
        #                                    cv2.VideoWriter_fourcc('M','J','P','G'), \
        #                                    30, \
        #                                    (f_width, f_height))
        log_file_name='log{ID}.txt'.format(ID=str(args.ID))
        try:
            os.remove(log_file_name)
        except: pass
        log_file = open(log_file_name, 'a')
        log_file.write('vehicle_id, count, frames, datetime\n')
        log_file.flush()
    ...
    while args.iscam or cap.get(cv2.CAP_PROP_POS_FRAMES) + 1 < cap.get(cv2.CAP_PROP_FRAME_COUNT):
        if ret:
            ...
            if args.record:
                ...
                for item in log:
                    _row = '{0}, {1}, {2}, {3}\n'.format('v_' + str(item['blob_id']), item['count'], item['frames'], item['datetime'])
                    log_file.write(_row)
                    log_file.flush()
